...
When `current_task_id` is present, the workflow manages the task lifecycle automatically unless a successful explicit `task_transition` step transitions that same task. In that case the explicit transition owns the final status, preventing a duplicate deferred write after `git_commit_owned`.
