...
func (b *recordingTaskBackend) Upsert(_ context.Context, req tasks.AddRequest) (leanMutationResult, error) {
	item := tasks.Task{ID: req.ID, Status: req.Status, Title: req.Title, ProjectionSource: "custom_backend"}
	b.items[item.ID] = item
	return leanMutationResult{Task: item, Source: "custom_backend"}, nil
}

func (b *recordingTaskBackend) SetStatus(_ context.Context, req tasks.StatusRequest) (leanMutationResult, error) {
	item := b.items[req.ID]
...
func (b *recordingTaskBackend) BatchUpsert(_ context.Context, req tasks.BatchUpsertRequest) (leanBatchMutationResult, error) {
	b.batched = true
	...
	return leanBatchMutationResult{Upserted: upserted, Source: "custom_backend", Validation: "test"}, nil
}

func (b *recordingTaskBackend) Delete(_ context.Context, req tasks.DeleteRequest) (leanMutationResult, error) {
	item := b.items[req.ID]
