...
func TestNewExposesAssistantGuidance(t *testing.T) {
	...
	for _, want := range []string{"long-term memory", "note action=list", "note action=read", "note action=search", "note action=update"} {
...
func TestRunSchemaIncludesWorkflowFields(t *testing.T) {
	...
	for _, field := range []string{"steps", "owned_files", "commit_message", "current_task_id", "task_on_start", "task_on_success", "task_on_failure", "secret_handles", "env", "vars", "stdin", "stdin_var"} {
