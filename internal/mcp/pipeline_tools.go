...
func registerPipelineTools(srv *server.MCPServer, deps *Server) {
	...
	srv.AddTool(basemcp.NewTool("run",
		runsCommands,
		basemcp.WithDescription("Run pipelines and workflows. Required: action, repo_path (except schema). Actions: pipeline (command, repo_path, cwd?, env?, vars?, stdin?/stdin_var?, secret_handles?, timeout_seconds?, mcp_wait_seconds?, current_task_id?, task_on_start?, task_on_success?, task_on_failure?, compact_output?) — run command with evidence extraction; workflow (repo_path, steps[], owned_files?, commit_message?, env?, vars?, secret_handles?, mcp_wait_seconds?, current_task_id?, task_on_start?, task_on_success?, task_on_failure?, preview?) — run guarded edits, checks, and optional commit; workflow env/vars apply to every command step (a step's own env/vars merge over them) and write_file content gets {{NAME}} substitution; execution is detached from this call: it waits up to mcp_wait_seconds (default and max 600), then answers running + workflow_id while the workflow keeps going server-side; workflow_status (workflow_id, wait_seconds?) — durable status and final result of a workflow run, blocking up to wait_seconds (max 600) instead of rerunning it; schema () — return valid workflow step types and parameters."),
		...
		basemcp.WithObject("env", basemcp.Description("Map of NAME to value merged into the environment of every command this run executes; reference as $NAME (always double-quoted). Not redacted — use secret_handles for secrets.")),
		basemcp.WithObject("vars", basemcp.Description("Map of NAME to value substituted as {{NAME}} into commands, stdin and write_file content before the shell or file layer sees them. Literal, non-recursive; {{{{ writes a literal {{; an unknown {{NAME}} fails closed listing known names.")),
		basemcp.WithString("stdin", basemcp.Description("String piped to the pipeline command's stdin instead of a heredoc (pipeline action).")),
		basemcp.WithString("stdin_var", basemcp.Description("Name of a var whose value is piped to the pipeline command's stdin (pipeline action); mutually exclusive with stdin.")),
		basemcp.WithArray("secret_handles", basemcp.Description("Server-config secret handles: each resolves to {{NAME}} substitution plus $NAME and $HELPER_SECRET_<HANDLE> in the environment, masked in output as [HELPER_SECRET:<HANDLE>]."), basemcp.WithStringItems()),
		...
		"step_tools": []map[string]any{
			{
				...
				"fields": map[string]string{
					"command":          "Shell command to run (string, required). {{NAME}} substitution applies with the step's vars merged over workflow vars.",
					"cwd":              "Optional repo-relative working directory (string).",
					"env":              "Optional map merged into this command's environment over the workflow env; reference as $NAME.",
					"vars":             "Optional map merged over workflow vars before {{NAME}} substitution.",
					"stdin":            "Optional string piped to the command's stdin instead of a heredoc.",
					"stdin_var":        "Optional var name whose value is piped to stdin; mutually exclusive with stdin.",
			...
			},
			{
				...
				"fields": map[string]string{
					"path":          "Repo-relative file path (string, required).",
					"content":       "Whole-file text content (string, required unless content_b64 is set). {{NAME}} substitution applies with workflow vars.",
