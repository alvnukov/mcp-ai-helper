...
11. Never set a task to done until its acceptance criteria, relevant gate, task status transition, and required owned-files commit are actually closed in one workflow; for a repo task with file changes, no such unified commit means the task is not done. A partial green test, timeout, evidence-only analysis success, skipped check, failed commit, post-hoc status commit, or unverified MCP/tool-facing path is not enough.
...
14. For project memory, read task_current or task_list before work, then synchronize the backlog with one task_batch_upsert call whenever you already know the complete authoritative task set; use close_missing only intentionally.
15. Never edit tasks by modifying task registry/source/projection files directly. Do not change MCPAIHelperProject/ActiveTasks.lean, tasks/*.lean, task JSON comments, or legacy task files to update task title/body/status/priority/tags/criteria/verification; use task_upsert, task_batch_upsert, task_set_status, task_delete, or another explicit task-facing helper tool only.
16. If task tools cannot express the needed task mutation, stop with a surface mismatch/blocker; do not bypass the helper with file edits, scripts, guarded_replace, shell, or direct git operations.
17. Use task statuses consistently: todo for planned work, in_progress for the current owner, blocked when external input is needed, and done for completed or intentionally closed work.
18. Prefer task_update or task_set_status for one targeted change; avoid long sequences of task_add calls when batch_upsert can express the desired state.
19. If a workflow cannot express the needed operation, improve the workflow DSL instead of normalizing repeated manual tool calls.
