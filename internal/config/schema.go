...
func Schema() map[string]any {
	return map[string]any{
		...
		"fields": []FieldDoc{
			...
			{Path: "layers.git_advanced.enabled", Type: "bool", Default: "false", Description: "Opt-in git history/inspection tools (blame, log, log_diff, branch_list, remote_list, stash_list, tag_list, prepare_task_worktree). Core git tools (status, diff, commit_owned) remain always available."},
			{Path: "layers.web.enabled", Type: "bool", Default: "false", Description: "Opt-in web research tools (web_search, web_fetch, fetched_doc_find, fetched_doc_read). Distinct from web_policy.enabled which gates network access."},
			{Path: "layers.task_advanced.enabled", Type: "bool", Default: "false", Description: "Opt-in task exploration tools (task_graph, task_context, task_export). Core task CRUD tools remain always available."},
			{Path: "layers.task_ui.enabled", Type: "bool", Default: "false", Description: "Opt-in local task browser HTTP UI tools (task_ui_start, task_ui_stop)."},
			{Path: "layers.config_advanced.enabled", Type: "bool", Default: "false", Description: "Opt-in feature flag tools (feature_list, feature_get, feature_enable, feature_disable, feature_reset)."},
			{Path: "layers.lake.enabled", Type: "bool", Default: "false", Description: "Opt-in Lean/Lake workspace tools (lake_init, lake_smoke)."},
			...
			{Path: "permissions.tools.deny", Type: "[]string", Description: "Repo-local only (.mcp-ai-helper.yaml). MCP tool names denied for LLM in this repo. User-editable; no helper tool writes repo configs."},
			{Path: "task_registry.backend", Type: "string", Default: "obsidian", Description: "Task registry backend selection. Empty or obsidian uses the Obsidian markdown registry under task_registry.obsidian.path (default obsidian-tasks, resolved against repo_path); lean keeps the legacy Lean/Lake registry. Any other value fails at startup.", Examples: []string{"lean", "obsidian"}},
			{Path: "task_registry.obsidian.path", Type: "string", Default: "obsidian-tasks", Description: "Obsidian task registry directory. Repo-relative paths are resolved against repo_path; the registry directory is created silently on first task access when missing."},
			{Path: "repo_config", Type: "file", Default: ".mcp-ai-helper.yaml (optional)", Description: "Per-repository config merged on top of global config. Feature tools may create/update only the features section and ensure this file is gitignored; no helper tool writes the rest of a repo config."},
		},
	}
}
