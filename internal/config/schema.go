...
func Schema() map[string]any {
	return map[string]any{
		...
		"workflow": []string{
			...
			"Use config_allow_repository to add the server-known startup repository to Jira, Confluence, or both; it never accepts an arbitrary path, and newly visible tools require a process restart.",
			...
			"After the user edits the helper config, use config_reload; config_allow_repository, config_option_set, and config_option_reset reload by default.",
		...
		"fields": []FieldDoc{
			...
			{Path: "integrations.jira.enabled", Type: "bool", Default: "true when jira config exists", Description: "Enables Jira before repository scoping is applied."},
			{Path: "integrations.jira.allowed_repositories", Type: "[]string", Default: "empty (disabled everywhere)", Description: "Global-only absolute path allowlist for local repositories that may see Jira tools. At least one explicit path is required; empty or missing denies every repository. Paths may start with ~; descendants are included. Unknown startup repository context is denied. Tool visibility changes after MCP restart."},
			{Path: "integrations.jira.allowed_projects", Type: "[]string", Default: "empty (all Jira projects)", Description: "Independent allowlist of Jira issue-key project prefixes such as VEGA."},
			{Path: "integrations.confluence.enabled", Type: "bool", Default: "true when confluence config exists", Description: "Enables Confluence before repository scoping is applied."},
			{Path: "integrations.confluence.allowed_repositories", Type: "[]string", Default: "empty (disabled everywhere)", Description: "Global-only absolute path allowlist for local repositories that may see Confluence tools. At least one explicit path is required; empty or missing denies every repository. Paths may start with ~; descendants are included. Unknown startup repository context is denied. Tool visibility changes after MCP restart."},
			{Path: "integrations.confluence.allowed_spaces", Type: "[]string", Default: "empty (all spaces)", Description: "Independent allowlist of Confluence space keys."},
